  ...
  const removeCommentCards = (startingPoint, isDelete = false) => {
    ...
    if (isDelete) {
      ...
      if (parentIndex !== undefined) {
        ...
        if (!commentsArr[parentIndex].childrenLevel.length) {
  ...
  const LoadMoreRepliesButton = () => {
    const button = (
      <button
        ...
        className="text-dark-grey p-2 px-3 hoverLbg-grey/30 border-md flex items-center gap-2"
      >
        Load More Replies
      </button>
    );
  ...
  return (
    <div className="w-full" style={{ paddingLeft: `${leftVal * 10}px` }}>
      <div className="my-5 p-6 rounded-md border border-grey">
        <div className="flex gap-3 times-center mb-8">
        ...
        <div className="flex gap-5 items-center mt-5">
          {commentData.isReplyLoaded ? (
            ...
            >
              <i className="fi fi-rr-comment-alt-middle"></i> Hide Reply
            </button>
          ) : (
            <button
              onClick={loadReply}
